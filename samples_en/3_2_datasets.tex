...
and fine-tuning, %szb: a training és fine-tuning az esetedben felteszem ugyanaz - akkor az and helytelen 
...
For this thesis, only the Java-to-C++ portion of XLCoST was used. %szb: csak a code translation? ha igen, azt is írd le.
The training notebooks %szb: ugyanígy, implementation detail nem kell, pl. hogy notebook
load the original dataset splits and filter them to retain only Java
...
One of the main advantages of XLCoST is its multilingual nature. %szb: ez most nagyon abszurd, nem értem hogy jön ide: épp az előbb írtad, hogy csak java-to-cpp-t használsz. Akkor miért jó, hogy amúgy multilingual?
Unlike smaller translation datasets that only support one or two language pairs,
XLCoST contains fine-grained parallel data across multiple languages and
supports ten different cross-lingual tasks. The dataset contains more than one
million parallel code pairs across all supported languages, making it one of the
largest publicly available datasets for cross-lingual code intelligence
\cite{zhu2022xlcost}. %szb: nem kell ismételgetni a cite-ot
...
thesis uses the CodeEditorBench framework %szb: ide kéne a cite
for execution-based evaluation.
...
runtime dependencies, and execution settings. %szb: ezt már mintha leírtad volna
