...
portion of failed translations. This suggests that many generated outputs still
contained syntax errors, missing headers, incompatible language constructs, or
incorrect memory handling.

When comparing the two models, Qwen consistently produced fewer
compilation errors and more accepted solutions than StarCoder2. This indicates
that Qwen was generally more successful at generating syntactically valid and
compilable C++ code.
...
This increase can be explained by the reduction in compilation errors. After
fine-tuning, more generated programs were able to compile successfully, which
made it possible for them to proceed to execution-based testing. As a result,
some programs that would previously have failed during compilation instead
failed due to incorrect logic or incomplete functionality.
...
from 15 to 9. This suggests that the fine-tuned Qwen model was more robust in
handling edge cases and avoiding runtime failures such as invalid memory access
or division by zero.
...
Overall, the execution-based evaluation demonstrates that fine-tuning improves
functional correctness for both model families. However, the results also show
that many remaining errors are related to logic and semantic correctness rather
than simple syntax issues. This confirms that generating compilable code is only
one aspect of successful code translation, while preserving the exact behavior
of the original program remains a more difficult challenge.
